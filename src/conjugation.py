			...
			for col in cols:
				...
				for tense_box in col.find_all('div', class_='blue-box-wrap', recursive=False):
					...
					for li in tense_box.find_all('li'):
						pronoun = ''.join(it.get_text() for it in li.find_all('i', class_="graytxt"))
						conj = ''.join(it.get_text() for it in li.find_all('i', class_="particletxt") + li.find_all('i', class_="auxgraytxt") + li.find_all('i', class_="verbtxt"))
...
# Get conjugation in target language
	...
	def highlight_conj(base: str, full_conj: str) -> Text:
		...
		- If identical to base: no highlighting
		- If different: highlight all differing characters
		"""
			...
			if data[mood]:
				for submood in data[mood]:
					...
					for conj in data[mood][submood]:
						...
						highlighted = highlight_conj(verb, data[mood][submood][conj])
